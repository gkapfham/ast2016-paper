...
\label{sec:empirical-study}
	...
	\subsection{Empirical Results}
		...
		\begin{figure}[t]
			...
			\caption{Scatter plot of time saved against number of tests (log y). \query{Some SQLite results are hidden as they have negative values and cannot be shown on a log scale. How should we handle this?}}
		\end{figure}

		\begin{figure}[t]
			\includegraphics[width=3.5in]{graphics/time_saved_vs_mutants_scatter_logy.pdf}
			\caption{Scatter plot of time saved against number of mutants (log y). \query{Some SQLite results are hidden as they have negative values and cannot be shown on a log scale. How should we handle this?}}
		\end{figure}

		\begin{figure*}[t]
			\centering
			\includegraphics[width=6in]{graphics/time_saved_vs_tests_scatter_noitrust_facetdbms.pdf}
			\caption{Scatter plot of time saved against number of tests (no iTrust, facet DBMS). \query{iTrust is excluded as it blows the scale. How should we handle this?}}
		\end{figure*}

		\begin{figure*}[t]
			\centering
			\includegraphics[width=6in]{graphics/time_saved_vs_mutants_scatter_noitrust_facetdbms.pdf}
			\caption{Scatter plot of time saved against number of mutants (no iTrust, facet DBMS). \query{iTrust is excluded as it blows the scale. How should we handle this?}}
		\end{figure*}
