...
\label{sec:empirical-study}
	...
	\begin{itemize}
		...
		\item \query{We have data for all 7 techniques (\Original, \FullSchemata, \JustInTimeSchemata, \UpFrontSchemata, \MinimalSchemata, \MinimalPlusSchemata, \VirtualMutationAnalysis). Which do we want to include?} \\
		\answer{\Original gives a good baseline. \MinimalSchemata was established as the best optimisation in the Mutation 2013 paper, however the current results show that for \SQLite, \JustInTimeSchemata is better. So, we should probably include \MinimalSchemata as a `best optimisation until now' and possibly \JustInTimeSchemata.}
