...
\begin{figure*}[t]
  ...
  {\small \justifying{ \noindent In this plot the box itself represents the interquartile range (IQR), or the measure of
      ...
      whisker goes from the bottom of the box to the lowest value within 1.5 times the IQR. The thick
      horizontal line represents the median value; since there is little variance in the timings for a given
      configuration, the boxes are noticeably compressed. Since the timings from running the original method on the
  \Postgres DBMS} \par}
